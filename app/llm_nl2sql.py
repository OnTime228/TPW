...
{
  "op": "count" | "sum",
  "metric": "videos" | "views" | "likes" | "comments" | "reports" | "delta_views" | "delta_likes",
  "filters": {
...
- Если не уверен — всё равно верни JSON по схеме.
"""
...
def _build_sql(obj: dict) -> Query:
    ...
    if isinstance(month, dict) and "year" in month and "month" in month:
        ...
        k1 = add_param(start)
        k2 = add_param(end)
        where_parts.append(f"{month_field} >= {k1}::date")
        where_parts.append(f"{month_field} < {k2}::date")
